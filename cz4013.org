...
- suppose want to query key k at node n, then lookup the table[n][lg(k-n)+1]
  ...
  8. let p be predecessor, s be successor
     - foreach i in [1..m],
       - foreach node x in [p-2^(i-1)+1,n-2^(i-1)],
         - table[x][i] = n
     lg(N) complexity of node insertion
...
- process state \in {PARTICIPANT, NON-PARTICIPANT}
...
- when process receives *election message*, compare message PID w/ own PID
  ...
  - if message PID < own PID and NON-PARTICIPANT, substitute & forward
  - if message PID < own PID and PARTICIPANT, stop
  - if message PID = own PID, be coordinator
- coordinator becomes NON-PARTICIPANT and sends *elected message* to neighbour, announce election, enclose identity
- when p_i receives election message
  - p_i becomes NON-PARTICIPANT
